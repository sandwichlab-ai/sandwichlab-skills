...
func newCmdProjectActivate(f *internal.Factory) *cobra.Command {
  ...
  ahcli ads project activate proj-xxx --schedule-interval 5m`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := f.RequireTenantID(&o.TenantID); err != nil {
				return err
			}
			...
			if o.ScheduleInterval != "" {
				params.Set("schedule_interval", o.ScheduleInterval)
			}
			client := o.f.AdsClient()
	...
	cmd.Flags().StringVar(&o.ScheduleInterval, "schedule-interval", "", "自定义调度间隔（如 5m, 1h, 24h），默认 24h")
	return cmd
}
